...
local SCAN_RATE = 20 -- scan rate in Hz (times a second) - higher scan rate requires more energy per tick
...
local SHOW_ALL_ENTITIES = false -- if true, will show mobs as well as players (requires more energy per tick)
  ...
  for k,v in pairs(radarScanReturn) do
    ...
    if tostring(MARK_MYSELF)=="true" or tostring(MARK_MYSELF)=="false" and radarScanReturn[k]["name"]~=userName then
      ...
      if INVERT_X_AXIS and not INVERT_Y_AXIS then
        ...
        xHistory = radarScanReturn[k]["x"]
        zHistory = radarScanReturn[k]["z"]
        nameHistory = radarScanReturn[k]["name"]
...
while running do
  ...
  local packetTable = {zHistory, xHistory, nameHistory}
  packet = serialization.serialize(packetTable)
  modem.broadcast(125, packet)
